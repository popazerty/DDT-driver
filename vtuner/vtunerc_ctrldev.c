/*
 * vtunerc: /dev/misc/vtunerc device
 *
					...
					size_t len, loff_t *off)
{
	...
	extern_inject_data(ctx->kernel_buf, len / 188);
...
int vtunerc_register_ctrldev(struct vtunerc_ctx *ctx)
{
	...
	for (idx = 0; idx < ctx->config->devices; idx++) {
		...
		clsdev = device_create(pclass, NULL,
				...
				/*ctx*/ NULL, "misc!vtunerc%d", idx);

		printk(KERN_NOTICE "vtunerc: registered /dev/misc/vtunerc%d\n",
				idx);
	}
